...
namespace GuideCube
{
    public class GCubeController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            defaultState = new GCSIdle();
        }

        private void Start()
        {
            SetState(new GCSGoTo(NearestPointPosition));
        }
        ...
        {
            Debug.Log($"{name}: Start state {state.GetType()}");
            currentState = state;
        ...
        private IEnumerator FollowTarget(float avaDistanceToTarget)
        {
            ...
            while (true)
            {
                ...
                if (path == null || Vector3.Distance(target, oldTarget) > 1)
                {
                    ...
                    if (path.Count > 1 && !(path[0] is PathPointWithPortal portalPoint && portalPoint.ConnectedPortalPoint == path[1]))
                    {
                ...
                if (distanceDisFrame > distanceToNextPathPoint)
                {
                    ...
                    // use portal
                    if (path[currentPathIndex - 1] is PathPointWithPortal pointWithPortal && path[currentPathIndex] == pointWithPortal.ConnectedPortalPoint)
                    {
